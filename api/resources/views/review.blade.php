...
@foreach($review as $rev)
    ...
    <tr>
            ...
            <td><span>{{$rev->review}}</span></td>
            <td><span>{{$rev->category}}</span></td>

            <td>
                <form action="/reviews-delete/{{$rev->id}}" method="POST" enctype="multipart/form-data">
                @csrf
            <button type="submit" class="delete-btn" onclick="return confirm('yakin ini menghapus testimoni ini?');">hapus</a>
                @method('DELETE')
        </form>
        </td>
